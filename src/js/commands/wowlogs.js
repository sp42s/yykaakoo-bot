...
export default class WowLogs {
    static async handleMessage(params) {
        if (params && params[0] === 'help') return 'Voit lisätä komennon perään parametrejä esim: guild=ryöstöretki realm=darksorrow region=eu'
    ...
    static getLatestRaid(params) {
        return new Promise((resolve, reject) => {
            ...
            if (query.guild && query.realm && query.region) {
                axios.get(`https://www.warcraftlogs.com:443/v1/reports/guild/${encodeURIComponent(query.guild)}/${query.realm}/${query.region}?api_key=${auth.warcraftlogsKey}`)
                    .then((res) => {
